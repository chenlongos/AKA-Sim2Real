...
class InferenceEventsMixin:
    async def on_act_infer(self, sid: str, payload: dict):
        ...
        logger.info(f"[{user_id}] 收到 ACT 推理请求")

        try:
            inference_state = payload.get("state", [0.0] * backend_config.STATE_DIM)
            image = payload.get("image")
            logger.info(f"[{user_id}] state: {inference_state}, image provided: {image is not None}")
            if image:
                logger.info(f"[{user_id}] image length: {len(image)}")

            action = self.sim_controller.infer(inference_state, image, user_id)
            await self.emit("act_infer_result", {"success": True, "action": action})
        except Exception as exc:
            logger.error(f"[{user_id}] ACT 推理失败: {exc}")
